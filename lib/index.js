...
const register = (server, options) => {
  ...
  if (options.rolling) {
    hoek.assert(options.expiresIn !== undefined, 'rolling requires expiresIn to be set');
  }
  ...
  const storeSession = async (request, h) => {
    if (!options.rolling && hoek.deepEqual(request.session, request._session)) {
      return h.continue;
    }
    ...
    if (!sessionId) {
      ...
      sessionId = createSessionId(hoek.base64urlDecode(sessionId, 'buffer').slice(0, options.size));
      h.state(options.name, sessionId);
    }
